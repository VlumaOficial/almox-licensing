-- ====================================================================
-- SCRIPT DE SETUP COMPLETO - MÓDULO 1 (COM DROP)
...
-- ====================================================================
-- 1. DROP DE TABELAS ANTIGAS (LIMPEZA COMPLETA)
-- ====================================================================

-- Dropar views primeiro (dependências)
...
-- Dropar triggers
DROP TRIGGER IF EXISTS trigger_update_last_validated ON validation_logs CASCADE;

-- Dropar funções
DROP FUNCTION IF EXISTS cleanup_old_logs() CASCADE;
DROP FUNCTION IF EXISTS check_expired_licenses() CASCADE;
DROP FUNCTION IF EXISTS update_last_validated() CASCADE;

-- Dropar tabelas (ordem reversa das dependências)
DROP TABLE IF EXISTS validation_logs CASCADE;
DROP TABLE IF EXISTS licenses CASCADE;
DROP TABLE IF EXISTS license_machines CASCADE;
DROP TABLE IF EXISTS installations CASCADE;
DROP TABLE IF EXISTS license_logs CASCADE;

-- Dropar políticas RLS
DROP POLICY IF EXISTS "Allow read access for validation" ON installations;
DROP POLICY IF EXISTS "Allow read access for validation" ON licenses;
DROP POLICY IF EXISTS "Allow insert for validation" ON validation_logs;
DROP POLICY IF EXISTS "Allow read access for validation" ON validation_logs;
...
-- ====================================================================
-- 9. DADOS DE TESTE (OPCIONAL - DESCOMENTAR SE NECESSÁRIO)
-- ====================================================================

/*
-- Inserir dados de teste para validação do Módulo 1
...
BEGIN
    ...
    WHERE table_schema = 'public'
    AND view_name IN ('validation_stats', 'active_licenses', 'expiring_trials');
    
    IF view_count = 3 THEN
        RAISE NOTICE 'SUCESSO: Todas as views foram criadas!';
    ELSE
        RAISE NOTICE 'AVISO: Views criadas: %', view_count;
    END IF;
END $$;
...
RAISE NOTICE 'Tabelas antigas dropadas e schema recriado.';
